...
def get_reportes_from_dev():
    ...
    developer = db.desarrollador.query.filter_by(id=id_dev).first()
    ...
    reportes = db.reporte.query.filter_by(id_developer=id_dev).all()
...
def get_likes_from_user():
    ...
    developer = db.desarrollador.query.filter_by(id=id_user).first()
    if developer is None:
        return jsonify({'error': 'No existe el usuario'})
    #get all the where in the table reporte the id_dev is the same as the id_dev in the request
    likes = db.like.query.filter_by(id_desarrollador=id_user).all()
    reportes = []
    for like in likes:
        id_reporte = like.id_reporte
        report = db.reporte.query.get_or_404(id_reporte)
...
def get_all_reports_and_products():
    ...
    if developer is None:
        return jsonify({'error': 'No existe el desarrollador'})
    product = db.producto.query.filter_by(id=id_product).first()
    if product is None:
    ...
    total_reports = len([reporte for reporte in db.reporte.query.filter_by(id_developer=id_dev).all() if reporte.id_estado != 0 and reporte.id_estado != 3])
    ...
    product_reports = len([reporte for reporte in db.reporte.query.filter_by(id_developer=id_dev).all() if reporte.id_estado != 0 and reporte.id_estado != 3 and reporte.id_producto == id_product])    
    #create a json with the information of the reportes
...
def get_all_reports_related_to_products():
    ...
    if db.desarrollador.query.filter_by(id=id_dev).first() == None:
    ...
    productos = db.desarrollador_producto.query.filter_by(id_desarrollador=id_dev).all()
    ...
    reportes = db.reporte.query.filter(db.reporte.id_producto.in_(id_productos)).all()
    #create a json with the information of the reportes
...
def get_user_reports():
    id_user = request.args.get('id_user')
    reportes = db.reporte.query.filter_by(id_cliente=id_user).all()
    reportes_json = [{'id': reporte.id, 'title': reporte.titulo, 'description': reporte.descripcion, 'likes': reporte.likes, 'date': reporte.fecha, 'id_estado': reporte.id_estado, 'id_prioridad': reporte.id_prioridad, 'id_producto': reporte.id_producto, 'id_developer' : reporte.id_developer} for reporte in reportes]
    return jsonify(reportes_json), 200   

async def add_desarrollador(nombre, email, id_rol):
    desarrollador = db.desarrollador(nombre, email, id_rol)
    db.session.add(desarrollador)
    db.session.commit()

async def add_cliente(nombre, email):
    cliente = db.cliente(nombre, email)
