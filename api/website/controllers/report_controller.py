...
def add_like():
    ...
    user = db.desarrollador.query.get_or_404(id_user)
    if db.like.query.filter_by(id_desarrollador=id_user, id_reporte=id_report).first() != None:
...
def get_report():
    ...
    if int(quantity) > db.reporte.query.filter_by(id_producto=id_product).count():
        ...
        return jsonify({'message': 'The quantity is bigger than the number of reports. Quantity asked is: '+ quantity+' The number of reports with the product id is: ' + str(db.reporte.query.filter_by(id_producto=id_product).count())}), 400
...
def add_comentario(descripcion, id_reporte):
    comentario = db.comentario(descripcion, id_reporte)
