...
const Reasignacion: React.FunctionComponent<IReasignacionProps> = (props) => {
  ...
  const fetchData = () => {
    fetch(
      "http://127.0.0.1:5000/reassignment/product/all?id_product=" + id_product,
    )
      .then((response) => {
  ...
  const reports = datos.map((reports: reporte) => {
    return {
      titulo: (
        <Button href={"/VerReporteEnv/" + reports.id_report} variant="link">
      ...
      asignacion: (
        <ReasignacionButton
          ...
          id_developer={reports.id_developer}
          developer_name={reports.developer_name}
          date={reports.date}
