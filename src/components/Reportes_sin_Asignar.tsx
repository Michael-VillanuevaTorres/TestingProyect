...
const Reportes_sin_Asignar: React.FunctionComponent<
  IReportes_sin_AsignarProps
> = (props) => {
  ...
  const filteredReports = datos.filter(
    (report: reporte) => report.id_state === 0,
  );

  const reports = filteredReports.map((reports: reporte) => {
    return {
      titulo: (
        <Button href={"/VerReporteEnv/" + reports.id} variant="link">
          {reports.title}
        </Button>
      ),
      ...
      prioridad_a: (
        <DropdownPrioridad id_report={reports.id}></DropdownPrioridad>
      ),
    };
  });
