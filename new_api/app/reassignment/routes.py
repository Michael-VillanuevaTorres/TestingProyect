...
def add_reassignment_petition():
    ...
    existing_reassignment = db.session.execute(db.select(Reassignment).filter_by(id_developer=id_developer,id_report = id_report)).first()
    ...
    return jsonify({'message': 'solicitud ya realizada'}), 400
